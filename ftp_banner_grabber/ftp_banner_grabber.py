...
def ret_banner(ip, port):
    try:
        ...
        s = socket.socket()
        s.connect((ip, port))
        print(Fore.YELLOW + f"[+] Established network connection with host {ip} using port {port}")
        banner = s.recv(1024)
        return f"{banner}"

    except SystemExit as sys_exit:
        print(Fore.LIGHTYELLOW_EX + f"[!] ERROR operation failed, system exit: {sys_exit}")
        return None

    except KeyboardInterrupt as keybd_err:
        print(Fore.LIGHTYELLOW_EX + f"[!] ERROR operation canceled via Keyboard Interrupt: {keybd_err}")
        return None

    except Exception as generic_err:
        print(Fore.LIGHTYELLOW_EX + f"[!] ERROR unable to establish socket connection to IP address {ip} on TCP port {port}: {generic_err}")
        return None
...
def main():
    ...
    if not target_ipv4_range:
        print(Fore.LIGHTYELLOW_EX + f'[!] ERROR - You must provide an IPv4 range in CIDR notation!')
        parser.print_help()
        exit(1)
    ...
    if not os.path.isfile(vulnerable_banner_file):
        print(Fore.LIGHTYELLOW_EX + f"[!] ERROR - {vulnerable_banner_file} does not exist!")
        exit(1)
    
    # Ensure we can read the file
    if not os.access(vulnerable_banner_file, os.R_OK):
        print(Fore.LIGHTYELLOW_EX + f"[!] ERROR - {vulnerable_banner_file} Access Denied.")
        exit(1)

    print(Fore.YELLOW + f"[+] INFO - Reading vulnerabilities from: {vulnerable_banner_file}")
