...
def ret_banner(ip, port):
    try:
        ...
        console.print(f"[!] ERROR operation failed, system exit: {sys_exit}")
        return None
...
def check_vulns(reference_file, test_banner):
    with open(reference_file, "r") as ref_file:
        for line in ref_file.readlines():
            if line.strip("\n") in test_banner:
                console.print(f"[+] Server is vulnerable! {test_banner}")
...
def main():
    ...
    if not target_ipv4_range:
        console.print(f"[!] ERROR - You must provide an IPv4 range in CIDR notation!")
        parser.print_help()
        sys.exit(1)
    ...
    if not target_ports:
        console.print(f"[!] ERROR - Port list not provided")
        parser.print_help()
        sys.exit(1)

    # Ensure we were provided a file as parameter
    if not vulnerable_banner_file:
        console.print(f"[!] ERROR - Vulnerable banner file not provided")
        parser.print_help()
        sys.exit(1)

    # Ensure file exists
    if not os.path.isfile(vulnerable_banner_file):
        console.print(f"[!] ERROR - {vulnerable_banner_file} does not exist!")
        sys.exit(1)

    # Ensure we can read the file
    if not os.access(vulnerable_banner_file, os.R_OK):
        console.print(f"[!] ERROR - {vulnerable_banner_file} Access Denied.")
        sys.exit(1)

    console.print(f"[+] INFO - Reading vulnerabilities from: {vulnerable_banner_file}")
